...
def build_model(mode, inputs, params, is_training):
    ...
    if params.model_version == 'lstm':
        ...
        #tf.Variable(tf.random_normal([2*params.lstm_num_units, att]))
        v   = tf.get_variable('v'  ,shape = [params.attention_units,  2*params.lstm_num_units],
                              initializer = tf.glorot_uniform_initializer())
        #tf.Variable(tf.random_normal([att  ,  2*params.lstm_num_units]))
        ...
        pool_output5M = tf.nn.pool(input=attention_output, window_shape=[5], pooling_type="MAX", padding="SAME")
        pool_output5A = tf.nn.pool(input=attention_output, window_shape=[5], pooling_type="AVG", padding="SAME")
        ...
        layer1_output = tf.layers.dense(mean_output, 50, activation = activation_fn)
        layer1_output = tf.nn.dropout(layer1_output, keep_rate)
        
        layer2_output = tf.layers.dense(layer1_output, 50, activation = activation_fn)
        layer2_output = tf.nn.dropout(layer2_output, keep_rate)
        
        layer3_output = tf.layers.dense(layer2_output, 25, activation = activation_fn)
        layer3_output = tf.nn.dropout(layer3_output, keep_rate)
        
        layer4_output = tf.layers.dense(layer3_output, 25, activation = activation_fn)
        layer4_output = tf.nn.dropout(layer4_output, keep_rate)
        
        layer5_output = tf.layers.dense(layer4_output, 25, activation = activation_fn)
        layer5_output = tf.nn.dropout(layer5_output, keep_rate)
