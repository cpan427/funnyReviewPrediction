...
def build_model(mode, inputs, params, is_training):
    ...
    if params.model_version == 'lstm':
        ...
        sentence = tf.layers.dense(sentence, 10, activation = tf.nn.relu, kernel_regularizer= tf.contrib.layers.l2_regularizer(l2_scale))
        ...
        layer1_output = tf.layers.dense(mean_output, 10, activation = activation_fn, kernel_regularizer= tf.contrib.layers.l2_regularizer(l2_scale))
        layer1_output = tf.nn.dropout(layer1_output, keep_rate)
        
        layer2_output = tf.layers.dense(layer1_output, 5, activation = activation_fn, kernel_regularizer= tf.contrib.layers.l2_regularizer(l2_scale))
